<template>
  <div class="flex items-center gap-x-2">
    <img 
      ...
      class="col-start-1 row-start-1 row-span-2 h-8"
      />
    <span class="row-start-1 col-start-2 text-sm">{{ timePerCatch !== undefined ? text : 'Fish will not be caught' }}</span>
  </div>
</template>
...
const text = computed(() => `${ Math.round(timePerRoeChest.value) } s/chestWithRoe | ${ Math.round(timePerRoe.value) } s/Roe | ${ timePerStack.value.toFixed(2) } h/Stack`)
</script>