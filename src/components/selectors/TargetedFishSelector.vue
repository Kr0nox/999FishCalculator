      ...
      <div class="relative flex flex-col gap-2">
        <div class="flex gap-2">
          ...
          <ContainerComponent
            ...
            @click="mainStore().prioritisedFish = []"
            >Remove All</ContainerComponent
          >
        </div>
        <div class="flex flex-col flex-wrap gap-2">
          <div
            v-for="fish in mainStore().prioritisedFish"
            :key="fish.Id"
...
const fishNames = computed(() => mainStore().prioritisedFish.map((f) => f.displayname))
...
function addFish(name: string) {
  ...
  if (!fish) return
  mainStore().prioritisedFish.push(fish)
}

function removeFish(fish: Fish) {
  mainStore().prioritisedFish = mainStore().prioritisedFish.filter((f) => f.Id != fish.Id)
}
