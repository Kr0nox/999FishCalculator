...
import { getJellyChance, nonFishItems, rollFishPoolWithTargetedBait, targetedBaitSingle } from "./lib/calculateChance.ts";
...
export function getFilteredFishData(c: InternalConfiguration) {
  ...
  const appendedFishData = getAppendedFishData(getFishFromLocationAndSeason(selectedLocation, selectedSeason))
  ...
  let correctSubArea = tempFishParamArray.filter(
    (fish) =>
      ...
      !fish.FishAreaId ||
      fish.FishAreaId === selectedSubArea
  );
  ...
  let distance = tempFishParamArray.filter(
    (fish) =>
      (fish.MaxDistanceFromShore <= -1 ||
        waterDepth <= fish.MaxDistanceFromShore) &&
  ...
  if (checkedItems.isTroutDerbyActive) {
    ...
    if (rainbowTrout[0])
      rainbowTrout[0].displayname = "Rainbow Trout (from event)";
  ...
  if (selectedSeason != "MagicBait") {
    let timeFilter = tempFishParamArray.filter(
      (fish) =>
        ...
        // double window fish
        (fish.time.length == 4 &&
          fish.time[2] <= timeOfDay &&
          fish.time[3] > timeOfDay)
    );
...
export function getChance(filteredFishData:any[], c: InternalConfiguration): CalculatorResults[] {
  ...
  const dailyLuck = c.dailyLuck
    ...
    // get jelly chance
    let jelly = filteredFishData.find(
      (jelly) => jelly.Id && jelly.Id.match(/Jelly/)
    );
    ...
    if (filteredFishData.length > 0) {
      ...
      if (checkedItems.isUsingTargetedBait && selectedSeason != "MagicBait") {
        ...
        for (let i in nonTargetedFish) {
          ...
          fish.finalChance = rollFishPoolWithTargetedBait(
            targetedBait,
            nonTargetedFish,
            i
          );
    ...
    function calculateWeight(fish: any) {
      ...
      if (!fish.IgnoreFishDataRequirements && fish.baseRate) {
        ...
        chanceFromFishData *=
          1 - Math.max(0, fish.maxDepth - waterDepth) * fish.depthMultiplier;
...
export function getChances(configuration: Configuration) {
  ...
  const endTime = configuration.startTime == configuration.endTime ? configuration.startTime + 1 : configuration.endTime;
  let d = 0
